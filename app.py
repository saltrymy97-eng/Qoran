...
/* ملاحظة: لا نخفي [data-testid="stBottom"] أبداً لأنه يحتوي على حقل الدردشة */
...
def smart_distribute_text(text):
    ...
    for line in text.split('\n'):
        ...
        # التحقق مما إذا كان السطر عنوانًا
        matched = False
...
# معالجة الأسئلة التلقائية من أزرار الخدمات
if st.session_state.auto_question:
    user_input = st.session_state.auto_question
    st.session_state.auto_question = None

if user_input:
    # 1. التحقق من الكود السري للأمان
    if user_input.strip() == ADMIN_SECRET_CODE:
        st.session_state.admin_mode = True
        st.rerun()

    # 2. في وضع الطالب: معالجة السؤال عبر الذكاء الاصطناعي
    ...
    # 3. في وضع الإدارة: لا معالجة، فقط تنبيه
    else:
